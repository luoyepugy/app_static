...
(function(root, factory) {
    ...
    $.fn.photoClip = function(option) {
        if (!window.FileReader) {
            mml.pup_ti("您的浏览器不支持 HTML5 的 FileReader API， 因此无法初始化图片裁剪插件，请更换最新的浏览器！","0");
            return;
        }
    ...
    function photoClip(container, option) {
        ...
        $file.change(function() {
            ...
            if (!/image\/\w+/.test(this.files[0].type)) {
            	mml.pup_ti("图片格式不正确，请选择正确格式的图片文件！",0);
                return false;
            } else if(filesize>3){
            	mml.pup_ti("图片大小控制在3M以内",0)
        		return;
        	}else{
